module "control_planes" {
  ...
  ssh_key_name                 = local.aws_key_pair_name
  ...
  security_group_ids           = each.value.disable_ipv4 && each.value.disable_ipv6 ? [] : [aws_security_group.k3s.id] # Cannot attach a firewall when public interfaces are disabled
  placement_group_id           = var.placement_group_disable ? null : (each.value.placement_group == null ? aws_placement_group.control_plane[each.value.placement_group_compat_idx].id : aws_placement_group.control_plane_named[each.value.placement_group].id)
  location                     = each.value.location
  server_type                  = each.value.server_type
  backups                      = each.value.backups
  subnet_id                    = aws_subnet.control_plane[[for i, v in var.control_plane_nodepools : i if v.name == each.value.nodepool_name][0]].id
  ...
  private_ipv4 = cidrhost(aws_subnet.control_plane[[for i, v in var.control_plane_nodepools : i if v.name == each.value.nodepool_name][0]].cidr_block, each.value.index + 101)
